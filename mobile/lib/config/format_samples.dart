...
///
/// Kept in step with `client/src/config/formatSamples.js`. If the parser
...
1. In the React component life cycle, the
...
2. Which hook manages local state in a
   function component?
   A. useEffect
   B. useState
   C. useMemo
  ...
  FormatSample(
    ...
    sample: '''
TEST II: TRUE OR FALSE (15 items, 1 point each)
...
3. A component name must begin with a
   capital letter.
   ANSWER: TRUE''',
    notes: [
  ...
  FormatSample(
    ...
    blurb:
        'Bubbled for the truth value, handwritten for the correction. Scored whole '
        'or split, whichever the exam is set to.',
    sample: '''
TEST III: MODIFIED TRUE OR FALSE
Directions: Write TRUE if correct. If FALSE,
give the word that makes it true.
...
3. The useMemo hook runs a side effect
   after render.
  ...
  FormatSample(
    ...
    blurb:
        'Handwritten on a ruled line, read by the handwriting pass and matched to '
        'your key.',
    sample: '''
TEST IV: IDENTIFICATION (10 items, 2 points each)
Directions: Write the term being described.

1. The library used to build user interfaces
   from components.
   ANSWER: ReactJS / React / React.js

2. The hook that runs a side effect after
   every render.
   ANSWER: useEffect''',
    notes: [
  ...
  FormatSample(
    ...
    blurb:
        'Handwritten. Also covers a code listing where the blanks are numbered '
        'inline.',
    sample: r'''
TEST V: FILL IN THE BLANKS (15 items, 1 pt each)
Directions: Write the missing word.

1. The ________ hook runs a side effect
   after every render.
   ANSWER: useEffect
...
useEffect(() => {
  ...
  FormatSample(
    ...
    sample: '''
TEST VII: ENUMERATION (3 items, 1 point each)
